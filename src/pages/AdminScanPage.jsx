...
import { LayoutDashboard, Package, ShoppingBag, QrCode, Camera, Search, Loader2, AlertCircle } from 'lucide-react';
...
export default function AdminScanPage() {
    ...
    setTimeout(async () => {
      try {
        ...
        await qrScanner.start({ facingMode: 'environment' }, { fps: 10, qrbox: { width: 220, height: 220 } },
          async (text) => { await stopScan(); const clean = text?.replace(/\r?\n|\r/g, '').trim() || ''; setScannedCode(clean); fetchItemDetails(clean); }, () => {});
  ...
  const fetchItemDetails = async (code) => {
    ...
    } catch { setErrorMsg('خطأ في الاتصال'); } finally { setLoading(false); }
  };
      ...
      <main style={{ padding: 16, maxWidth: 500, margin: '0 auto' }}>
        <form onSubmit={handleManualSearch} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: 12, marginBottom: 12 }}>
          <div style={{ display: 'flex', gap: 8 }}>
            <input type="text" placeholder="أدخل رمز الباركود..." value={manualCode} onChange={e => setManualCode(e.target.value)} style={{ flex: 1, padding: '10px 14px', borderRadius: 10, border: '1px solid #cbd5e1', fontSize: 14, outline: 'none' }} />
            <button type="submit" style={{ background: '#2563eb', color: '#fff', border: 'none', borderRadius: 10, padding: '10px 16px', fontWeight: 800, cursor: 'pointer' }}><Search size={16} /> بحث</button>
          </div>
        </form>
        ...
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: 16, textAlign: 'center' }}>
          {!scanning && !item && !loading && (
            <div>
              ...
              <button onClick={startScan} style={{ background: '#2563eb', color: '#fff', border: 'none', borderRadius: 10, padding: '12px 24px', fontWeight: 800, cursor: 'pointer', marginTop: 12 }}><Camera size={18} /> بدء الكاميرا</button>
            </div>
          )}
          ...
          {errorMsg && <div style={{ background: '#fef2f2', color: '#b91c1c', padding: 12, borderRadius: 10, margin: '12px 0' }}><AlertCircle size={18} /> {errorMsg}</div>}
          {item && (
            <div style={{ textAlign: 'right', background: '#f8fafc', borderRadius: 12, padding: 16, border: '1px solid #e2e8f0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}><strong>{item.name}</strong><span style={{ padding: '4px 10px', borderRadius: 20, fontSize: 12, background: item.status === 'available' ? '#d1fae5' : '#fee2e2', color: item.status === 'available' ? '#047857' : '#b91c1c' }}>{item.status === 'available' ? 'متوفر' : 'مباع'}</span></div>
              <div style={{ fontSize: 13 }}>SKU: <code>{item.sku}</code> | رف: {item.shelf} | باركود: <code>{item.barcode}</code></div>
              <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                <button onClick={toggleItemStatus} style={{ flex: 1, background: item.status === 'available' ? '#d97706' : '#2563eb', color: '#fff', border: 'none', borderRadius: 8, padding: '10px', fontWeight: 800, cursor: 'pointer' }}>{item.status === 'available' ? 'بيع' : 'إرجاع'}</button>
                <button onClick={() => { setItem(null); setManualCode(''); }} style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 8, padding: '10px', fontWeight: 700, cursor: 'pointer' }}>مسح آخر</button>
              </div>
            </div>
          )}
        </div>
