...
export default function HomePage() {
  ...
  const features = [
    ...
    { icon: Truck, title: 'توصيل لـ 58 ولاية', description: 'توصيل سريع إلى باب المنزل أو المقر عبر شركات توصيل موثوقة.' },
    { icon: Wrench, title: 'دعم تقني متخصص', description: 'فريق تقني يساعدك في التأكد من توافق رقم البوردة.' },
  ...
  return (
    <div style={{ background: 'var(--bg-primary, #0f172a)', color: 'var(--text-primary, #f8fafc)', direction: 'rtl', minHeight: '100vh', fontFamily: "'Cairo', sans-serif" }}>
      ...
      <header style={{ position: 'relative', height: '420px', overflow: 'hidden' }}>
        {slides.map((slide, index) => (
          <div key={index} style={{ position: 'absolute', inset: 0, backgroundImage: `url(${slide.image})`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: currentSlide === index ? 1 : 0, transition: 'opacity 1s ease-in-out' }}>
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, var(--bg-primary, #0f172a) 5%, rgba(15,23,42,0.7) 50%, rgba(15,23,42,0.85) 100%)' }} />
          </div>
        ))}
        <div style={{ position: 'relative', zIndex: 10, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 16px' }}>
          <h1 style={{ fontSize: 'clamp(24px, 5vw, 38px)', fontWeight: 900, marginBottom: '8px', color: '#fff', textShadow: '0 2px 10px rgba(0,0,0,0.6)' }}>{slides[currentSlide].title}</h1>
          <p style={{ fontSize: '15px', color: '#cbd5e1', marginBottom: '20px', textShadow: '0 1px 5px rgba(0,0,0,0.6)' }}>{slides[currentSlide].subtitle}</p>
          <Link to="/store" className="btn btn-accent btn-lg" style={{ gap: 6 }}>تصفح القطع المتوفرة <ChevronLeft size={18} /></Link>
          <div style={{ display: 'flex', gap: 8, marginTop: 20 }}>
            {slides.map((_, index) => (
              <button key={index} onClick={() => setCurrentSlide(index)} style={{ width: currentSlide === index ? 24 : 8, height: 8, borderRadius: 4, border: 'none', background: currentSlide === index ? 'var(--accent)' : 'rgba(255,255,255,0.4)', cursor: 'pointer', transition: 'all 0.3s' }} />
            ))}
          </div>
        </div>
      </header>
      ...
      <section style={{ marginTop: '-40px', position: 'relative', zIndex: 20, padding: '0 16px 20px 16px' }}>
        <div className="card" style={{ maxWidth: '850px', margin: '0 auto', padding: '24px', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.3)' }}>
      ...
      <section style={{ background: 'var(--bg-secondary)', padding: '40px 16px' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <h2 className="reveal" data-reveal style={{ fontSize: '22px', fontWeight: 900, textAlign: 'center', marginBottom: '6px' }}>تصنيفات القطع</h2>
          <p className="reveal" data-reveal style={{ textAlign: 'center', color: 'var(--text-secondary)', fontSize: '13px', marginBottom: '28px' }}>تصفح القطع حسب النوع المراد تصليحه</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            {categories.map((cat) => {
              const Icon = cat.icon;
              return (
                <button key={cat.key} onClick={() => navigate(`/store?category=${cat.key}`)} className="card reveal" data-reveal style={{ padding: '28px 16px', textAlign: 'center', cursor: 'pointer', transition: 'all 0.2s', color: 'inherit' }}
                  ...
                  <div className="icon-box icon-box-lg" style={{ background: `${cat.color}18`, color: cat.color, margin: '0 auto 14px auto', width: '56px', height: '56px' }}><Icon size={28} /></div>
      ...
      <section style={{ background: 'var(--bg-primary)', padding: '40px 16px' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <h2 className="reveal" data-reveal style={{ fontSize: '20px', fontWeight: 900, textAlign: 'center', marginBottom: '24px' }}>لماذا يفضل التقنيون DZBoard؟</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            {features.map((f, i) => {
              const Icon = f.icon;
              return (
                <div key={i} className="card reveal" data-reveal style={{ padding: '24px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div className="icon-box icon-box-md" style={{ background: 'rgba(99,102,241,0.12)', color: 'var(--primary)', marginBottom: '12px', width: '44px', height: '44px' }}><Icon size={22} /></div>
                  <h3 style={{ fontSize: '14px', fontWeight: 'bold', marginBottom: '6px' }}>{f.title}</h3>
                  <p style={{ fontSize: '12px', lineHeight: '1.5', color: 'var(--text-secondary)', margin: 0 }}>{f.description}</p>
                </div>
              );
            })}
          </div>
      ...
      <section style={{ background: 'var(--bg-secondary)', padding: '36px 16px' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <h2 className="reveal" data-reveal style={{ fontSize: '20px', fontWeight: 900, textAlign: 'center', marginBottom: '20px' }}>ندعم القطع لأشهر الماركات العالمية والمحلية</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '12px' }}>
            {brands.map((brand, index) => (
              <button key={index} onClick={() => navigate(`/store?brand=${brand.code}`)} className="card reveal" data-reveal style={{ padding: '14px 8px', textAlign: 'center', cursor: 'pointer', transition: 'all 0.2s', color: 'inherit', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}
      ...
      <section className="reveal" data-reveal style={{ background: 'var(--bg-primary)', padding: '40px 16px', textAlign: 'center' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '13px', marginBottom: '20px' }}>تواصل مع فريق الدعم الفني لدينا مباشرة للتأكد من توفر القطعة البديلة لجهازك.</p>
          <Link to="/store" className="btn btn-accent btn-lg" style={{ gap: 8 }}>تصفح كافة القطع الآن <ChevronLeft size={18} /></Link>
        </div>
      </section>

      <footer style={{ background: 'var(--bg-sidebar)', color: 'var(--text-sidebar)', borderTop: '1px solid var(--border)', padding: '32px 16px 20px 16px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
          <div style={{ marginBottom: '8px', fontSize: '18px', fontWeight: 900, color: '#fff' }}>DZ<span style={{ color: 'var(--accent)' }}>Board</span></div>
          <p style={{ fontSize: '12px', lineHeight: '1.6', marginBottom: '16px', opacity: 0.8 }}>متجر قطع غيار الشاشات الأول في الجزائر - توصيل لـ 58 ولاية والدفع عند الاستلام.</p>
          <Link to="/store" style={{ color: 'inherit', textDecoration: 'none', fontSize: '12px' }}>المتجر</Link>
      ...
      <a href="https://m.me/dzboard" target="_blank" rel="noopener noreferrer" style={{ position: 'fixed', bottom: '24px', left: '24px', width: '56px', height: '56px', borderRadius: '50%', background: 'linear-gradient(135deg, #0084FF, #00C6FF)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 14px rgba(0,132,255,0.45)', zIndex: 40, transition: 'all 0.3s', color: '#fff' }}
