      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '16px' }}>
        ...
        <div className="card" style={{ padding: 16, marginBottom: 16 }}>
            ...
            {categories.map((cat) => {
              ...
              return (
                <button key={cat.key} onClick={() => setSelectedCategory(cat.key)}
                  className={`btn ${selectedCategory === cat.key ? 'btn-primary' : 'btn-ghost'} btn-sm`} style={{ gap: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            <select value={selectedBrand} onChange={(e) => setSelectedBrand(e.target.value)} className="field-input" style={{ width: 'auto', minWidth: 130 }}>
              ...
              <div key={product.id} className="card" style={{ overflow: 'hidden', cursor: 'pointer', transition: 'all 0.2s', display: viewMode === 'list' ? 'flex' : 'block', gap: viewMode === 'list' ? 16 : 0 }}
                ...
                <div style={{ padding: 14, flex: 1 }}>
                  <div style={{ display: 'flex', gap: 6, marginBottom: 6, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 6, background: `${getCategoryColor(product.category)}18`, color: getCategoryColor(product.category), fontWeight: 700 }}>
                      {getCategoryName(product.category)}
                    </span>
                    {product.stock <= 3 && product.stock > 0 && (
                      <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 6, background: 'rgba(245,158,11,0.12)', color: '#f59e0b', fontWeight: 700 }}>آخر {product.stock} قطع</span>
                    )}
                    {product.stock === 0 && (
                      <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 6, background: 'rgba(239,68,68,0.12)', color: '#ef4444', fontWeight: 700 }}>غير متوفر</span>
                    )}
                  </div>
