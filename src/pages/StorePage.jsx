      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '12px 16px', position: 'sticky', top: 0, zIndex: 30 }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <Link to="/" style={{ color: '#3b82f6', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}><ChevronLeft size={18} /> الرئيسية</Link>
            <h1 style={{ fontSize: 18, fontWeight: 900 }}>المتجر</h1>
          </div>
          <button onClick={() => setViewMode(viewMode === 'grid' ? 'list' : 'grid')} style={{ background: 'none', border: '1px solid #e2e8f0', padding: 8, borderRadius: 8, cursor: 'pointer', color: '#64748b' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
            {categories.map((cat) => (
              <button key={cat.key} onClick={() => setSelectedCategory(cat.key)}
                style={{ padding: '8px 14px', borderRadius: 8, border: selectedCategory === cat.key ? '2px solid #3b82f6' : '1px solid #e2e8f0', background: selectedCategory === cat.key ? '#eff6ff' : '#fff', color: selectedCategory === cat.key ? '#3b82f6' : '#64748b', cursor: 'pointer', fontWeight: 600, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {loading ? (
          ...
          <div style={viewMode === 'grid' ? { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 } : { display: 'flex', flexDirection: 'column', gap: 12 }}>
            {products.map((product) => (
              <div key={product.id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, overflow: 'hidden', cursor: 'pointer', transition: 'all 0.2s', display: viewMode === 'list' ? 'flex' : 'block', gap: viewMode === 'list' ? 16 : 0 }}
                ...
                <div style={{ padding: 14, flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <button onClick={() => handleBuyNow(product)} disabled={product.stock === 0} style={{ padding: '8px 16px', background: product.stock ? '#f59e0b' : '#e2e8f0', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, cursor: product.stock ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: 6 }}>
