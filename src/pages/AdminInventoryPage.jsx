...
export default function AdminInventoryPage() {
  ...
  const [form, setForm] = useState({ name: '', shelf: '', position: '', price: '', image: '', category: 'parts', brand: 'generic' });
  ...
  const handleAdd = async () => {
    ...
    setShowAdd(false);
    setForm({ name: '', shelf: '', position: '', price: '', image: '', category: 'parts', brand: 'generic' });
    loadItems();
  };
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '16px' }}>
        ...
        {showAdd && (
          <div className="card" style={{ padding: 16, marginBottom: 16 }}>
            <h3 style={{ marginBottom: 12 }}>إضافة قطعة جديدة (تظهر في المتجر تلقائياً)</h3>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
              <input className="field-input" style={{ flex: 2, minWidth: 200 }} placeholder="اسم المنتج *" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
              <input className="field-input" style={{ width: 120 }} placeholder="السعر (دج)" type="number" value={form.price} onChange={e => setForm({...form, price: e.target.value})} />
              <input className="field-input" style={{ width: 100 }} placeholder="الرف" value={form.shelf} onChange={e => setForm({...form, shelf: e.target.value})} />
              <input className="field-input" style={{ width: 80 }} placeholder="الموقع" value={form.position} onChange={e => setForm({...form, position: e.target.value})} />
              <input className="field-input" style={{ width: 200 }} placeholder="رابط الصورة" value={form.image} onChange={e => setForm({...form, image: e.target.value})} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
          {[
            { label: 'متوفر', status: 'available', color: '#10b981', bg: '#d1fae5' },
            { label: 'مباع', status: 'sold', color: '#ef4444', bg: '#fee2e2' },
          ].map(s => {
            const count = items.filter(i => i.status === s.status).length;
            return (
              <div key={s.status} className="card" style={{ padding: 12, textAlign: 'center', background: s.bg, borderColor: s.color }}>
                <div style={{ fontSize: 22, fontWeight: 900, color: s.color }}>{count}</div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #e2e8f0' }}>
                  <th style={{ padding: 10 }}>صورة</th>
                  <th style={{ padding: 10 }}>SKU</th>
                  <th style={{ padding: 10 }}>باركود</th>
                  <th style={{ padding: 10 }}>المنتج</th>
                  <th style={{ padding: 10 }}>السعر</th>
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    <td style={{ padding: 10 }}>
                      <img src={item.image || 'https://via.placeholder.com/40'} style={{ width: 40, height: 40, borderRadius: 6, objectFit: 'cover' }} />
                    </td>
                    <td style={{ padding: 10, fontWeight: 700 }}>{item.sku}</td>
                    <td style={{ padding: 10, fontFamily: 'monospace', fontSize: 12 }}>
                      {item.barcode}
                      <button onClick={() => window.open(`https://barcode.tec-it.com/barcode.ashx?data=${item.barcode}&code=Code128`, '_blank')} style={{ marginRight: 6, background: 'none', border: 'none', cursor: 'pointer' }}>🖨️</button>
                    </td>
                    <td style={{ padding: 10 }}>{item.name}</td>
                    <td style={{ padding: 10 }}>{item.price ? `${item.price} دج` : '-'}</td>
                    <td style={{ padding: 10 }}>{item.shelf} - {item.position}</td>
                    <td style={{ padding: 10 }}>
